...
if (CLIENT) then
	function ITEM:paintOver(item, w, h)
		...
		if (item:getData("durability")) then
			...
			draw.SimpleText(item:getData("durability") .. "%", "nutIconFont", x, y, Color((255 / 100) * (100 - durability), (255 / 100) * durability, 0, 255))
...
ITEM.functions.Equip = {
	...
	onRun = function(item)
		...
		for k, v in pairs(items) do
			if (v.id != item.id) then
				...
				if itemTable.eqslot == "Shield" and itemTable:getData("equip") == true and item.type == "2h" then
	...
	onCanRun = function(item)
		return (!IsValid(item.entity) and item:getData("equip") != true and item:getData("durability") > 0)
	end
}
...
ITEM.functions.Repair = {
	...
	onRun = function(item)
		...
		if items:hasItem("remnabor") then
			client:getChar():getInv():remove(client:getChar():getInv():hasItem("remnabor"):getID())

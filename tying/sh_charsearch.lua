...
nut.command.add("charsearch", {
	onRun = function(client, arguments)
		...
		local searchTime = 10 - (client:getChar():getAttrib("stm", 0) / 200)
		
		if (IsValid(target) and target:IsPlayer() and target:getNetVar("restricted")) then
			client:setAction("Обыск...", searchTime)
		...
		elseif (IsValid(target.player)) then
			client:setAction("Обыск...", searchTime)
			client:doStaredAction(target, function()
				PLUGIN:searchPlayer(client, target.player)
			end, searchTime, function()
				if IsValid(client) then
					client:setAction()
				end
